...
int main(void){
    ...
    if ( trans == 'N' ){
        float input[2 + lda*n + (1 + ( n - 1 )*abs( incx )) + (1 + ( m - 1 )*abs( incy ))];
        ...
        x = malloc( (1 + ( n - 1 )*abs( incx )) * sizeof(float));
        ...
        for ( i = 0; i < 1 + ( m - 1 )*abs( incy ); ++i ){
            y[i] = input[2+lda*n+1 + ( n - 1 )*abs( incx )+i];
        }
    }
    else{
        float input[2 + lda*n + (1 + ( m - 1 )*abs( incx )) + (1 + ( n - 1 )*abs( incy ))];
        klee_make_symbolic(&input, sizeof(input), "input");
        ...
        x = malloc( (1 + ( m - 1 )*abs( incx )) * sizeof(float));
        ...
        for ( i = 0; i < 1 + ( n - 1 )*abs( incy ); ++i ){
            y[i] = input[2+lda*n+1 + ( m - 1 )*abs( incx )+i];
        }
    }
